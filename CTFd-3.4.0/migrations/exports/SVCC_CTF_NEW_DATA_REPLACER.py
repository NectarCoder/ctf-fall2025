...
if not os.path.isdir('db') or not os.path.isdir('uploads'):
	print()
	print('ERROR: Cannot find "db" and "uploads" directories.\nThis script must be ran on the outside of an extracted CTF instance.')
	sys.exit()

pathToChallengesJson = os.path.join('db', 'challenges.json')
pathToFlagsJson = os.path.join('db', 'flags.json')



print('Please select the Google Form (Responses) document:')
responsesInputFileName = selectFile(r'.*\.csv', False)


readerFile = open(responsesInputFileName, 'r')
reader = csv.reader(x.replace('\0', '') for x in readerFile)

header = next(reader)

if 'Timestamp' not in header[0] or 'Email Address' not in header[1]:
	print('ERROR: First column is not "Timestamp" and second is not "Email Address"...')
	print(f'First column: "{header[0]}"')
	print(f'Second column: "{header[1]}"')
	sys.exit()
